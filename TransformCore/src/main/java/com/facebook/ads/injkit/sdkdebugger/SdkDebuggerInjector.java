...
class SdkDebuggerInjector extends BaseInjector {
    ...
    private SdkDebuggerInjector(
            String loggerIName,
            Class<? extends Annotation> logCallAnnotationClass) {
        super(__ -> false);
    ...
    private static Class<? extends Annotation> loadAnnotation(ClassLoader classLoader, String name)
        ...
        try {
            ...
            if (!checkIfMethodExists(METHOD_DESCRIPTION_PARAM, cls)) {
                throw new InvalidAnnotationFormatException(String.format(
                        Locale.US,
    ...
    public static BaseInjector make(
            URLClassLoader applicationCode,
            SdkDebuggerConfiguration config)
            throws InvalidAnnotationProcessorConfigurationException {
        if (!config.isEnabled()) {
            ...
            while (visibleIt.hasNext()) {
                ...
                if (nextDesc.equals(
                        AsmNameUtils.classJavaNameToDescriptor(
                                logCallAnnotationClass.getName()))) {
                    visibleIt.remove();
                    return annotation;
                }
            }
    ...
    private void injectLogCall(MethodNode method, String description) {
        MethodInsnNode logCallMethod =  new MethodInsnNode(
