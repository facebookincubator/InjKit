...
public class LocalClassLoaderTest {
    ...
    private static final String RESOURCE_PATH =
            LocalClassLoaderTest.class.getName().substring(
                    0,
                    LocalClassLoaderTest.class.getName().lastIndexOf('.')).replace('.', '/')
                    + "/testdatafile";
    ...
    private static ClassLoader findRootClassLoader() {
        ...
        for (
                rootClassLoader = LocalClassLoader.class.getClassLoader();
        ...
        try (ZipOutputStream zipOutput = new ZipOutputStream(new FileOutputStream(tmp))) {
            ...
            try (InputStream clsRsrc =
                         LocalClassLoaderTest.class
                                 .getClassLoader()
                                 .getResourceAsStream(resourceOfClass(cls))) {
    ...
    private ClassLoader makeLocalLoaderForClass(Class<?> cls, ClassLoader parent)
            throws IOException {
    ...
    public void before() throws Exception {
        ...
        urlsWithoutTestCode = removeUrlThat(allUrls, urls -> {
        ...
        urlsWithoutTestResource = removeUrlThat(allUrls, urls -> {
            URLClassLoader tmpLoader = new URLClassLoader(urls, rootClassLoader);
    ...
    public void failsToLoadClassIfIsNeitherInParentNorInUrl() throws Exception {
        ClassLoader ldr =
                makeLocalLoaderForClass(
                        ClassNoOneLoads.class,
                        makeStdLoaderForClass(ClassNoOneLoads.class, rootClassLoader));
