...
public class AnnotationProcessorInvoker {
    ...
    public static void invoke(Task compileTask, File configurationFile) throws Exception {
        if (!AnnotationProcessorInvoker.class
            ...
            LocalClassLoader localLoader =
                    new LocalClassLoader(
                            currentClassLoader.getURLs(),
                            currentClassLoader.getParent());
            Class<?> invokerClass =
                    localLoader.loadClass(AnnotationProcessorInvoker.class.getName());
        ...
        AnnotationProcessorConfigurationBuilder builder =
    ...
    private static Iterable<File> getBootstrapClasspath(Task compileTask) throws Exception {
        return ((FileCollection) invokeGetter(
                invokeGetter(compileTask, "getOptions"), "getBootstrapClasspath")).getFiles();
    }
}
